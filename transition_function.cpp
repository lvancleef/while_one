...
void Transition_Function::load(ifstream& definition, Input_Alphabet input_alphabet, States states, Stack_Alphabet stack_alphabet, bool& valid) {
	...
	// 			// Stack Alphabet
	// 			if (!(stack_alphabet.is_element(transLines[i])) && i == 2) {
	...
	// 			i++;
	// 			if (!(stack_alphabet.is_element(transLines[i])) && i == 4) {
	// 				cout << "Error: Transition Function contains a character not from Stack Alphabet." << endl;
	// 				valid = false;
	// 				return;
	// 			}
...
void Transition_Function::validate (const Stack_Alphabet& stack_alphabet,
	...
	// 		valid = false;
	// 	}
	// 	if (!states.States::is_element(transitions[i].destinations[destVectIndex].destination_state())) {
	// 		cout << "Destination State " << transitions[i].destinations[destVectIndex].destination_state() << " is not in states." << endl;
	// 		valid = false;
	// 	}
	// 	if (!stack_alphabet.Stack_Alphabet::is_element(transitions[i].destinations[destVectIndex].write_string())) {
	// 		cout << "Write String" << transitions[i].destinations[destVectIndex].write_string() << " is not in stack alphabet." << endl;
	// 		valid = false;
	// 	}
...
void Transition_Function::view() const {
	...
	for (int i = 0; i < (int)transitions.size(); i++) {
		...
		cout << "delta("<< transitions.at(i).source_state() << ", " << transitions.at(i).read_character() << ", "<<
				...
				<< transitions.at(i).destinations[destVectIndex].destination_state() << ", "
				<< transitions.at(i).destinations[destVectIndex].write_string() << ")";
		destVectIndex++;
	}
}
...
void Transition_Function::find_transition(string source_state, char read_character,
	...
	// 			&& (transitions[i].read_stack() == read_stack_character)) {
	// 		destinations = transitions[i].destinations;
	// 		found = true;
	// 		return;
	// 	}
	// }
